...
namespace GUI
{
    public partial class ucNhanSu : UserControl
    {
        ...
        public ucNhanSu()
        {
            ...
            LoadData();
            ToggleInputPanel(false); // Ẩn form nhập liệu ban đầu
        }
        ...
        void SetupCustomControls()
        {
            ...
            // B. RIGHT PANEL - Cấu hình lại controls trong pnlInput
            // Lấy lại các control từ Designer (đã được tạo thông qua CreateInputGroup)
            // Do đã dùng code-behind cho Input Group trong Designer, nên chỉ cần chỉnh sửa thuộc tính
            txtTenNV.ReadOnly = false;
            txtSDT.ReadOnly = false;
            txtDiaChi.ReadOnly = false;
        }

        // Tái tạo Top Panel bằng code
        private Panel CreateTopPanel()
        {
            Panel pnl = new Panel { Dock = DockStyle.Top, Height = 70, BackColor = Color.White };
            pnl.Controls.Add(this.btnAdd);
            pnl.Controls.Add(this.btnSua);
            pnl.Controls.Add(this.btnXoa);
            pnl.Controls.Add(this.btnSearch);
            return pnl;
        }

        // Hàm hỗ trợ tạo nhãn + ô nhập liệu nhanh (Giữ lại để sử dụng trong Designer)
        ...
        void LoadData()
        {
            ...
            // Đặt tên cột tiếng Việt và thiết lập UIHelper
            string[] headers = { "Mã NV", "Họ và tên", "Điện thoại", "Địa chỉ", "Trạng thái TK" };
            string[] dataFields = { "MaNV", "Ten", "SDT", "DiaChi", "TrangThai" }; // TrangThai là cột mới từ join NguoiDung

            UIHelper.SetGridColumns(dgvNV, headers, dataFields);

            // Ẩn các cột không cần thiết
            if (dgvNV.Columns.Contains("TrangThaiTK")) dgvNV.Columns["TrangThaiTK"].Visible = false;
        }

        // Hàm mở form nhập liệu (Slide in/out)
        void ToggleInputPanel(bool open)
        {
            pnlInput.Width = open ? 300 : 0;
        }

        // Hàm mở Form Input/Update
        void OpenInputForm(string maNV)
        {
            currentMaNV = maNV;
            ToggleInputPanel(true);
            btnUnlock.Visible = false; // Mặc định ẩn nút mở khóa
            ...
            {
                // Sửa/Xem thông tin
                lblInputTitle.Text = "THÔNG TIN/CẬP NHẬT NV";
                btnLuu.Text = "Lưu cập nhật";
                txtTenNV.ReadOnly = true; // KHÔNG CHO PHÉP SỬA TÊN

                if (dgvNV.CurrentRow != null)
                {
                    txtTenNV.Text = dgvNV.CurrentRow.Cells["Ten"].Value.ToString();
                    txtSDT.Text = dgvNV.CurrentRow.Cells["SDT"].Value.ToString();
                    txtDiaChi.Text = dgvNV.CurrentRow.Cells["DiaChi"].Value.ToString();

                    // Hiển thị nút mở khóa nếu tài khoản đang bị Khóa
                    string trangThai = dgvNV.CurrentRow.Cells["TrangThai"].Value.ToString();
                    if (trangThai == "Khóa")
                    {
        ...
        private void DgvNV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                // Kích hoạt OpenInputForm ở chế độ Sửa khi click vào lưới
                OpenInputForm(maNV);
            }
        }
        ...
        private void BtnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txtTenNV.Text) || string.IsNullOrEmpty(txtSDT.Text))
            {
                MessageBox.Show("Họ tên và SĐT không được để trống!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (currentMaNV == null)
            {
                // Thêm mới (BLL hiện tại trả về thông báo chưa hỗ trợ)
                msg = bll.AddStaff(nv);
            }
            else
            {
        ...
        private void BtnUnlock_Click(object sender, EventArgs e)
        {
            if (dgvNV.CurrentRow != null)
            {
                // Lấy Tên Đăng Nhập (SĐT) từ dòng hiện tại
                ...
                {
                    string msg = bll.UnlockStaff(tenDN);
                    MessageBox.Show(msg);
                    if (msg.Contains("mở khóa"))
                    {
                        LoadData();
                        ToggleInputPanel(false);
                    }
