...
namespace GUI
{
    public partial class ucBan : UserControl
    {
        ...
        void BtnThanhToan_Click(object sender, EventArgs e)
        {
            ...
            // Tính tổng tiền từ Label hoặc tính lại từ lưới
            decimal tongTien = 0;
            if (!decimal.TryParse(lblTongTien.Text.Replace(" VNĐ", "").Replace(".", ""), NumberStyles.Currency, viVN, out tongTien))
            {
                MessageBox.Show("Lỗi tính tổng tiền. Vui lòng thử lại.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // Lấy Mã Hóa Đơn hiện tại (cần thiết cho form thanh toán)
            ...
            {
                // Nếu form thanh toán thành công (DialogResult.OK)
                // -> Logic ResetAndHideRightPanel() đã được thực thi
                ResetAndHideRightPanel();
            }
        }
